# 通道集成对比：crew-rs vs OpenClaw

crew-rs 与 OpenClaw 消息平台集成方式的详细对比。

## 1. 核心抽象

### crew-rs：扁平 Trait（12 个方法）
...
### crew-rs 独有：纯 Rust 密码学

crew-rs 使用纯 Rust 实现密码学原语（SHA-1、SHA-256、AES-128-CBC、AES-256-CBC、HMAC、Base64），用于飞书、企业微信、Twilio 的 webhook 签名验证。无 OpenSSL 依赖，服务单一二进制部署模型。

### crew-rs 独有：WhatsApp 桥接架构

```
WhatsApp ←→ Node.js 桥接（Baileys）←→ WebSocket ←→ crew-rs
                                            ↓
...
crew-rs 仅 Telegram 有 `markdown_to_telegram_html()` 转换，其他平台发送原始文本。
...
crew-rs 机器人在 Telegram 群组中会回应每条消息。这是显著的用户体验问题。提及门控是高优先级改进。
