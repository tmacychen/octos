...
本文档评估使用 Matrix 服务器作为 octos 的统一消息桥接中枢，让所有外部消息平台（Telegram、WhatsApp、Discord 等）通过 Matrix 桥接连接到 octos，而非直接 API 集成。

**结论**：Matrix 中枢方案不建议替代 octos 现有的直接集成，但作为**可选附加通道**很有价值——适用于小众平台和已有 Matrix 基础设施的用户。
...
octos 只需实现一个 Matrix 通道适配器。所有外部平台通过 Matrix 桥接（mautrix 系列）连接。每个外部用户表现为幽灵用户（如 `@telegram_12345:server`）。
...
5. octos 的 Matrix 客户端通过 `/sync` API 接收消息
6. octos 处理后在 Matrix 房间回复
7. 桥接接收回复并发送回原平台
...
| **资源占用** | Synapse + 桥接约 2 GB RAM | 包含在 octos 二进制中 |
...
octos 已有 Telegram、WhatsApp、飞书、Twilio、企业微信的完整适配器。切换到 Matrix 桥接会：
...
为 octos 实现 `MatrixChannel` 适配器提供两个好处：

1. **已有 Matrix 基础设施的用户**可通过其服务器连接 octos——已运行的桥接直接可用
