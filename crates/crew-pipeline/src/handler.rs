...
use crew_core::{AgentId, Task, TaskContext, TaskKind, TokenUsage};
...
/// Runs a full crew-agent Agent loop at the node.
...
pub struct CodergenHandler {
    ...
    provider_policy: Option<crew_agent::ToolPolicy>,
...
impl CodergenHandler {
    ...
    pub fn with_provider_policy(mut self, policy: Option<crew_agent::ToolPolicy>) -> Self {
...
impl Handler for CodergenHandler {
    async fn execute(&self, node: &PipelineNode, ctx: &HandlerContext) -> Result<NodeOutcome> {
        ...
        let mut tools = crew_agent::ToolRegistry::with_builtins(&self.working_dir);
        ...
        let policy = crew_agent::ToolPolicy {
        ...
        let config = crew_agent::AgentConfig {
            max_iterations: 30,
        ...
        let worker =
            crew_agent::Agent::new(worker_id.clone(), provider, tools, self.memory.clone())
                .with_config(config)
