...
pub fn build() -> WorkflowInstance {
    WorkflowInstance {
        ...
        additional_instructions: "You are a background research analyst. Follow the runtime-owned workflow phases in order: research, draft_report, deliver_result. Use run_pipeline with an inline DOT graph to complete the research in the background. Deliver exactly one final user-facing report. Do not emit intermediate status chatter or send intermediate files.".to_string(),
    }
}
