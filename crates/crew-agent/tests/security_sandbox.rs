...
//!
//! Run with: `cargo test -p crew-agent --test security_sandbox`
...
/// Helper: build a crew-rs-style SBPL profile for a workspace.
fn crew_sbpl(workspace: &str, allow_network: bool) -> String {
...
/// Helper: build SBPL with restricted reads (like crew-rs read_allow_paths).
...
fn should_allow_write_inside_workspace() {
    ...
    let profile = crew_sbpl(&real_workspace, false);
...
fn should_block_write_outside_workspace() {
    ...
    let (code, _stdout, stderr) = run_sandboxed(
        &profile,
        "echo 'escape' > /tmp/crew-security-test-escape.txt",
    );
...
fn should_block_write_to_etc() {
    ...
    let (code, _stdout, stderr) =
        run_sandboxed(&profile, "echo 'pwned' > /etc/crew-security-test.txt");
...
fn should_block_write_to_home() {
    ...
    let (code, _stdout, _stderr) = run_sandboxed(
        &profile,
        "echo 'escape' > ~/crew-security-test-escape.txt 2>&1 || echo BLOCKED",
    );
    // Either fails (exit 1) or we see BLOCKED
    let (_, stdout2, _) = run_sandboxed(
        &profile,
        "test -f ~/crew-security-test-escape.txt && echo EXISTS || echo MISSING",
    );
    assert!(
...
fn should_block_tmp_write_without_tmp_allowance() {
    ...
    // crew-rs SBPL without /private/tmp allowance (the fix)
    let profile = crew_sbpl(&real_workspace, false);
    let (code, _stdout, stderr) = run_sandboxed(
        &profile,
        "mkdir -p /tmp/crew-security-test-tmp && echo 'escape' > /tmp/crew-security-test-tmp/evil.txt",
    );
...
"#,
    ...
    let (code, _stdout, _stderr) = run_sandboxed(
        &profile,
        "echo 'escape' > /tmp/crew-security-test-loophole.txt",
    );
    assert_eq!(
        code, 0,
    ...
    // Clean up
    let _ = std::fs::remove_file("/tmp/crew-security-test-loophole.txt");
}
...
import os
try:
    os.makedirs('/tmp/crew-security-test-python', exist_ok=True)
    open('/tmp/crew-security-test-python/evil.txt', 'w').write('escape')
    print('BAD: escaped')
...
fn should_block_cross_user_write() {
    ...
    // User A's sandbox
    let profile = crew_sbpl(&real_a, false);
...
fn should_isolate_two_users_simultaneously() {
    ...
    let profile_a = crew_sbpl(&real_a, false);
    ...
    let profile_b = crew_sbpl(&real_b, false);
...
fn should_restrict_reads_when_configured() {
    ...
    let config = crew_agent::SandboxConfig {
        enabled: true,
        mode: crew_agent::sandbox::SandboxMode::Macos,
        allow_network: false,
        docker: crew_agent::sandbox::DockerConfig::default(),
        read_allow_paths: vec!["/usr".into(), "/opt/custom".into()],
    };
    let sandbox = crew_agent::create_sandbox(&config);
    let dir = tempfile::tempdir().unwrap();
...
fn should_reject_absolute_paths() {
    use crew_agent::tools::resolve_path;
...
fn should_reject_symlink_in_read_no_follow() {
    ...
    let result = rt.block_on(crew_agent::tools::read_no_follow(&link));
...
fn should_reject_symlink_in_write_no_follow() {
    ...
    let rt = tokio::runtime::Runtime::new().unwrap();
    let result = rt.block_on(crew_agent::tools::write_no_follow(&link, b"overwrite"));
...
fn should_block_private_ips() {
    use crew_agent::tools::ssrf::is_private_ip;
...
fn should_block_private_hosts() {
    use crew_agent::tools::ssrf::is_private_host;
...
fn should_block_dangerous_docker_cwd() {
    use crew_agent::sandbox::DockerConfig;

    // Test via the actual sandbox module
    let sb = crew_agent::sandbox::create_sandbox(&crew_agent::SandboxConfig {
        enabled: true,
        mode: crew_agent::sandbox::SandboxMode::Docker,
        allow_network: false,
