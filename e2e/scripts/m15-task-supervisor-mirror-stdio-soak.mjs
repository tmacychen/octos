...
function rpc(method, params = {}, rpcTimeoutMs = 10_000) {
  const id = `m15-task-mirror-${++nextSeq}-${crypto.randomBytes(3).toString('hex')}`;
...
async function main() {
  ...
  for (const method of ['agent/list', 'agent/status/read', 'task/output/read']) {
  ...
  await waitFor(
    () => agentUpdated.some(mirroredAgent),
    'mirrored TaskSupervisor agent/updated notification',
  );

  const mirroredUpdates = agentUpdated.filter(mirroredAgent);
  ...
  const agentId = terminalUpdate.agent.agent_id;
  ...
  assert(
    listed.backend_kind === terminalUpdate.agent.backend_kind,
    'agent/list backend_kind mismatch',
  );
  assert(listed.status === terminalUpdate.agent.status, 'agent/list terminal status mismatch');

  const agentStatus = await rpc('agent/status/read', {
    agent_id: agentId,
    session_id: sessionId,
    profile_id: profileId,
  });
  assert(
    agentStatus?.agent?.backend_kind === terminalUpdate.agent.backend_kind,
    'agent/status backend_kind mismatch',
  );

  const taskId = terminalUpdate.agent.task_id;
  assert(taskId, 'mirrored agent did not expose the source task_id');
  const taskOutput = await rpc('task/output/read', {
    session_id: sessionId,
    task_id: taskId,
    cursor: { offset: 0 },
  });
  assert(
    String(taskOutput?.text || '').includes('fixture output line one'),
    'task/output/read did not return deterministic fixture output',
  );
  ...
  const summary = {
    ...
    listedAgent: {
      agentId: listed.agent_id,
      backendKind: listed.backend_kind,
      status: listed.status,
    },
    taskOutputBytes: taskOutput.text.length,
    stderrPreview: stderrText.split(/\r?\n/).filter(Boolean).slice(-20),
    host: os.hostname(),
  };
  writeJson(path.join(runRoot, 'm15-task-supervisor-mirror-summary.json'), summary);
...
main()
  .catch((error) => {
    ...
    writeJson(path.join(runRoot, 'm15-task-supervisor-mirror-summary.json'), failure);
