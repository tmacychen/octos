...
fs.writeFileSync(
  path.join(workspace, 'context_restart_fixture.txt'),
  'Context restart fixture workspace file.\n',
);
...
async function startFakeOpenAiServer() {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    const content = `Context restart proof response ${requests.length}. ${finalMarker}`;
    if (body.stream === true) {
      res.writeHead(200, {
...
async function main() {
  ...
  try {
    ...
    const firstTurns = [];
    for (let i = 0; i < preRestartTurns; i += 1) {
      const turnId = await runTurn(
        first,
        `Force context compaction before model call, pre-restart turn ${i + 1}/${preRestartTurns}. ${'alpha beta gamma '.repeat(pressureRepeat)}`,
      );
      const status = await first.rpc('session/status/read', {
        session_id: sessionId,
        profile_id: profileId,
      });
      const context = contextStateOf(status);
      assert(context, `pre-restart turn ${i + 1} status missing context_state`);
      assert(context.last_compaction_id, `pre-restart turn ${i + 1} missing last_compaction_id`);
      firstTurns.push({ turnId, context });
    }
    const firstContext = firstTurns[firstTurns.length - 1].context;
    assert(firstContext, 'first status missing context_state');
    assert(firstContext.last_compaction_id, 'first status missing last_compaction_id');
    assert(firstContext.transcript_hash, 'first status missing transcript_hash');
    assert(firstContext.recovery_state === 'exact', `first recovery_state was ${firstContext.recovery_state}`);
    await first.close();
    ...
    assert(
      ...
      `restart should load exact context ledger, got ${secondInitialContext.recovery_state}`,
    );
    assert(
      secondInitialContext.last_compaction_id === firstContext.last_compaction_id,
      `restart changed compaction id: ${firstContext.last_compaction_id} -> ${secondInitialContext.last_compaction_id}`,
    );
    assert(
      secondInitialContext.transcript_hash === firstContext.transcript_hash,
      `restart changed transcript hash: ${firstContext.transcript_hash} -> ${secondInitialContext.transcript_hash}`,
    );
    const secondTurns = [];
    for (let i = 0; i < postRestartTurns; i += 1) {
      const turnId = await runTurn(
        second,
        `Continue after exact context reconnect, post-restart turn ${i + 1}/${postRestartTurns}. ${'delta epsilon zeta '.repeat(pressureRepeat)}`,
      );
      ...
      assert(context.recovery_state === 'exact', `post-restart turn ${i + 1} recovery_state was ${context.recovery_state}`);
      assert(context.last_compaction_id, `post-restart turn ${i + 1} missing last_compaction_id`);
      secondTurns.push({ turnId, context });
    }
    const secondContext = secondTurns.length > 0
      ? secondTurns[secondTurns.length - 1].context
    ...
    writeJson(path.join(runRoot, 'm16-context-restart-stdio-summary.json'), summary);
    ...
    writeJson(path.join(runRoot, 'm16-context-restart-stdio-summary.json'), failure);
